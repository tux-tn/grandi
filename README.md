...
NDI™ was conceived as a grand vision for IP media transport. The earliest bindings leaned into that idea with the tongue‑in‑cheek name “gra‑NDI‑ose”. **Grandi** preserves the homage: it nods to “[grandiose](https://github.com/Streampunk/grandiose)”, itself drawn from the French word _grandi_ (“grown”).
...
```ts
{
  ...
  colorFormat: 100, // grandi.ColorFormat.Fastest
  bandwidth: 100,   // grandi.Bandwidth.Highest
...
const receiver = await grandi.receive({
	...
	bandwidth: grandi.Bandwidth.AudioOnly,
	allowVideoFields: true,
	name: "rooftop",
});
```
...
```ts
{
  ...
  frameFormatType: grandi.FrameType.Interlaced,
  timecode: 0n,
...
```ts
const prefs = { audioFormat: grandi.AudioFormat.Float32Separate };
...
console.log(tally.onProgram, tally.onPreview);
```
...
1. **Install prerequisites**
   ...
   - Git LFS is _not_ required; the NDI SDK is fetched dynamically.
...
| `default`                                                        | `typeof grandi`      | The `grandi` object containing the methods above together with `ColorFormat`, `AudioFormat`, `Bandwidth`, `FrameType`, `FourCC`, and `TIMECODE_SYNTHESIZE`.                   |
...
Properties: `source`, `colorFormat`, `bandwidth`, `allowVideoFields`, `name`.
...
Properties: `name`, `groups`, `clockVideo`, `clockAudio`.
...
| `sourceName()`           | `string`  | Current source name presented to the network.   |
...
| `SenderTally`         | `{ changed: boolean; onProgram: boolean; onPreview: boolean; }`                                                    | Returned by `sender.tally()`. `on_program` and `on_preview` remain available as deprecated aliases.                                              |
...
- `ColorFormat` — enumerates `BGRX_BGRA`, `UYVY_BGRA`, `RGBX_RGBA`, `UYVY_RGBA`, `Fastest`, `Best`, and `BGRX_BGRA_FLIPPED`. Access via the `ColorFormat` enum (e.g. `grandi.ColorFormat.Fastest`).
- `AudioFormat` — `Float32Separate`, `Float32Interleaved`, `Int16Interleaved`. Access via the `AudioFormat` enum.
- `Bandwidth` — `MetadataOnly`, `AudioOnly`, `Lowest`, `Highest`. Access via the `Bandwidth` enum.
- `FrameType` — `Interlaced`, `Progressive`, `Field0`, `Field1`. Access via the `FrameType` enum.
