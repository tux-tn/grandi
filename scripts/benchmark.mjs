...
function createColors(enabled) {
  const useColor =
    enabled && process.stdout.isTTY && process.env.NO_COLOR == null && process.env.TERM !== "dumb";

  const wrap = (open, close) => (text) => (useColor ? `${open}${text}${close}` : String(text));
...
function summarizeLatencies(latenciesMs, count) {
  if (count === 0) {
    return { count: 0 };
  }
  const sorted = new Float64Array(count);
  for (let i = 0; i < count; i++) sorted[i] = latenciesMs[i];
  sorted.sort();
  let sum = 0;
...
async function waitForSourceByName(grandi, name, timeoutMs = 15_000) {
  ...
  try {
    while (Date.now() < deadline) {
      finder.wait(250);
      const match = finder.sources().find((source) => source.name.includes(name));
  ...
  try {
    ...
    const receiverTask = (async () => {
      ...
      while (controller.running && Date.now() < deadlineAtMs) {
        ...
        const frame = args.framesync ? await fs.video() : await receiver.data(50);
        if (frame.type === "timeout") continue;

        if (frame.type === "video") {
          if (frame.xres !== expectedSize.xres || frame.yres !== expectedSize.yres) {
            continue;
          }
    ...
    const sendVideoAvgMs = sendVideoCount ? toMs(sendVideoNs) / sendVideoCount : 0;
    const sendAudioAvgMs = sendAudioCount ? toMs(sendAudioNs) / sendAudioCount : 0;
    ...
    if (latency.count) {
      ...
      console.log(
        `${c.bold("- video latency:")} ${c.dim("no samples (no video frames received)")}`,
      );
    }
  } finally {
    try {
