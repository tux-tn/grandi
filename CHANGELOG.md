...
### Changed
- Discovery options now use `extraIPs`; `extraIps` remains accepted as a deprecated alias.
- `AudioFrame` properties `noChannels` and `noSamples` renamed to `channels` and `samples`; old names remain accepted as deprecated aliases
